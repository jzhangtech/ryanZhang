			...
			switch($channel->type){
				case "facebook":
					if(!is_numeric($channel->id)):
						$fb = json_decode($this->readData("https://graph.facebook.com/".$channel->id));
						if(isset($fb->id))
							$channel->id = $fb->id;
					endif;

					array_push($write,"$"."nss->addChannel(
						array(
							'type'=>'facebook',
							'id'=>'{$channel->id}',
							'access_token'=>'{$channel->access_token}',
							'access_token_expires'=>'{$channel->expires}',
							'limit'=>{$channel->limit},
							'show_all'=>{$channel->show}
						)
					);");
					break;
				case "twitter":
					array_push($write,"$"."nss->addChannel(
						array(
							...
							'id'=>'{$channel->id}',
							'access_token'=>'{$channel->access_token}',
							'access_token_secret'=>'{$channel->secret}',
							'limit'=>{$channel->limit}
						)
					);");
					break;
